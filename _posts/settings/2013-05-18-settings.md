...
[Read more about timesheet tags](../../settings/tags/#using_timesheet_tags).

* Name of the device used to clock in. Use this if if you manage multiple locations (each with their own time clock) and want to compare costs between locations. It can also be used as a naive method of job tracking, where each time clock is set up as a separate job in your accounting system.
...
* Tag applied to employee's roster. Use this if you want the timesheet tag to match up to the [roster tag](../../settings/tags/#using_roster_tags). For example, if you have lots of locations but only a few time clocks, you can use roster tags to classify locations and have this data copy across to timesheets. If there are no rosters for this shift, no tag will be added (you can add one manually).
...
#### Allow employees to manage allowances

If enabled, employees are able to edit their [shift allowances](../../timesheets/allowances/) when vieiwng their timesheets.
...
You can use [tags](../../settings/tags) to freely categorise shifts on rosters. Sometimes you will need to add a second level of categorisation based on a department. Enable this setting (which is disabled by default) to do so.
...
When you set up a [tracking category](http://help.xero.com/help/Settings_Tracking.htm#BK_SetUpCategory) in Xero, you create one or more options for that category. Here you should enter the default tracking option. When PayAus exports your timesheets to Xero, it uses the shift's [tag](../../settings/tags/#using_timesheet_tags) to attempt to match to a tracking category option. If it is not able to match one, the default option is used.
