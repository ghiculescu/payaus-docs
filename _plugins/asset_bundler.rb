...
END
        ...
        if context.registers[:site].config['asset_bundler'].key?('dev')
          ret_config['dev'] = context.registers[:site].config['asset_bundler']['dev'] ? true : false
        elsif context.registers[:site].config['server']
          ret_config['dev'] = true
        end
